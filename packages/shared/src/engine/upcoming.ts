...
/**
 * Advance a date by one frequency period, preserving day-of-month anchoring
 ...
 * the next N days.
 *
 * @param templates - All recurring templates (only active ones are used)
 * @param daysAhead - Number of days to look ahead (default 30)
 * @param today - Reference date (defaults to current date)
 * @returns Projected upcoming transactions sorted by date ascending
 */
export function getUpcomingTransactions(
  ...
  const todayDate = today ? parseDate(today) : new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate());
