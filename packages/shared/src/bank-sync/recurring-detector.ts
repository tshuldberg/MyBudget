...
function detectFrequency(dates: string[]): FrequencyResult {
  ...
  // Check frequency bands with tolerance
  const bands: Array<{
  ...
  for (const band of bands) {
    if (Math.abs(med - band.target) <= band.tolerance) {
      // Calculate how many intervals fall within the tolerance
...
export function detectRecurringCharges(
  ...
  for (const group of groups) {
    ...
    // Skip if we could not detect any pattern
    ...
    // Confidence: weighted combination of interval regularity and amount consistency
    let confidence =
