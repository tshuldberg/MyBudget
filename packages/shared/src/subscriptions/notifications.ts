 ...
 *
 * Determines which notifications should fire and logs them in
 * the notification_log table for deduplication. The actual scheduling
...
import type { NotificationLog, Subscription } from '../models/schemas';
 ...
 * Returns notification(s) that should be scheduled if not already sent.
 *
...
export function getRenewalNotifications(
  ...
  // Only generate if the notify date is today or in the future
 ...
 * Generates alerts at 3 days and 1 day before trial_end_date.
 *
 * @param db - database adapter
 * @param subscription - the subscription to check
 * @param today - reference date (defaults to current date)
 ...
 * @param monthlyTotalCents - the total monthly cost in cents
 */
export function getMonthlySummaryNotification(
  ...
  // Use a placeholder subscription_id for monthly summaries.
  // The notification_log UNIQUE constraint is (subscription_id, type, scheduled_for).
...
export function logNotification(
  ...
  db.execute(
    `INSERT OR IGNORE INTO notification_log (id, subscription_id, type, scheduled_for, sent_at)
     VALUES (?, ?, ?, ?, ?)`,
...
export function cancelNotifications(
  ...
  db.execute(
    `DELETE FROM notification_log WHERE subscription_id = ? AND sent_at IS NULL`,
    [subscriptionId],
  );
}
...
export function getNotificationLog(
  ...
  const rows = db.query<Record<string, unknown>>(
    `SELECT * FROM notification_log WHERE subscription_id = ? ORDER BY scheduled_for DESC`,
    [subscriptionId],
  );
